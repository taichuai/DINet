...
if __name__ == "__main__":
    ...
    training_data_loader = DataLoader(dataset=train_data, batch_size=opt.batch_size, shuffle=False, drop_last=True,num_workers=8,pin_memory=True,sampler=train_sampler, prefetch_factor=1)
    ...
    net_g = DDP(net_g, device_ids=[local_rank], output_device=local_rank)
    net_g = convert_model(net_g)
    net_dI = DDP(net_dI, device_ids=[local_rank], output_device=local_rank)
    net_dV = DDP(net_dV, device_ids=[local_rank], output_device=local_rank)
    # net_vgg = DDP(net_vgg, device_ids=[local_rank], output_device=local_rank)
    # net_lipsync = DDP(net_lipsync, device_ids=[local_rank], output_device=local_rank)
    ...
    for epoch in range(opt.start_epoch, opt.non_decay+opt.decay+1):
        ...
        for iteration, data in enumerate(training_data_loader):
            ...
            print(
                "===> Epoch[{}]({}/{}):  Loss_DI: {:.4f} Loss_GI: {:.4f} Loss_DV: {:.4f} Loss_GV: {:.4f} Loss_perception: {:.4f} lr_g = {:.7f} time_cost_per_step: {:.2f}s/it".format(
        ...
        if epoch %  opt.checkpoint == 0 and dist.get_rank() == 0:
            ...
            states = {
                'epoch': epoch + 1,
                'state_dict': {'net_g': net_g.module.state_dict(),'net_dI': net_dI.module.state_dict(),'net_dV': net_dV.module.state_dict()},
...
# python -m torch.distributed.launch --nproc_per_node 3 train_DINet_clip_ddp.py --augment_num=3 --mouth_region_size=256 --batch_size=6 --pretrained_frame_DINet_path=./asserts/training_model_weight/xxx/xxx.pth --result_path=./asserts/training_model_weight/clip_training_256_ddp
